...
def settings():
    ...
    if request.method == 'POST':
        ...
        # MA3 Plugin-Pfad
        ma3_plugin_path = request.form.get('ma3_plugin_path', '').strip()
        session['ma3_plugin_path'] = ma3_plugin_path
        gdtf_api.clear_cache()  # Cache leeren bei neuen Credentials
        saved = True
    gdtf_user = session.get('gdtf_user', '')
    gdtf_password = session.get('gdtf_password', '')
    autosave_interval = session.get('autosave_interval', '0')
    ma3_plugin_path = session.get('ma3_plugin_path', '')
    return render_template('settings.html', 
                           gdtf_user=gdtf_user, 
                           gdtf_password=gdtf_password,
                           autosave_interval=autosave_interval, 
                           ma3_plugin_path=ma3_plugin_path,
                           saved=saved,
...
def dashboard():
    # Im Lite-Mode: Auto-Login (kein Login-Screen nötig)
    if current_app.config.get('LITE_MODE') and 'user' not in session:
        session['user'] = 'LiteUser'
...
def login():
    # Im Lite-Mode: Auto-Login, kein Login-Screen
    if current_app.config.get('LITE_MODE'):
        session['user'] = 'LiteUser'
        return redirect(url_for('main.dashboard'))
